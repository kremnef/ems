...
<!--+
    ...
    |
    | @version $Id: forms-field-styling.xsl 629377 2008-02-20 07:18:10Z antonio $
    +-->
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="head" mode="forms-field">
        ...
        <script type="text/javascript">
            ...
            cocoon = {};
            <xsl:value-of
                    select="concat('cocoon.formsResourcesUri = ', $doubleQuote, $forms-resources, $doubleQuote, ';')"/>
        </script>
        <script src="{$dojo-resources}/dojo.js" type="text/javascript"/>
        <!-- load dojo -->
        <script type="text/javascript">dojo.require("dojo.widget.*");</script>
        <!-- require dojo.widget for auto-loading -->
        <xsl:if test="$dojo-debug = 'true'">
            <!-- require console etc. for dojo debug, if requested -->
            <script type="text/javascript">dojo.require("dojo.debug.console"); dojo.require("dojo.widget.Tree");
            </script>
        </xsl:if>
        <script src="{$forms-resources}/js/forms-lib.js" type="text/javascript"/>
        <!-- load legacy scripts -->
        <script type="text/javascript">                                            <!-- load forms library -->
            <xsl:value-of
                    select="concat('dojo.registerModulePath(', $doubleQuote, 'cocoon.forms', $doubleQuote, ', ', $doubleQuote, $forms-resources, '/js', $doubleQuote, ');')"/>
            <!-- tell dojo how to find our forms module. NB: (since 2.1.11, replaces cocoon.js) -->
        ...
        <!--<script type="text/javascript">
            var djConfig = {bindEncoding: "utf-8"};
            <xsl:if test="$dojo-debug = 'true'">                                           &lt;!&ndash; turn on debugging, if requested &ndash;&gt;
                <xsl:text> djConfig.isDebug = true; </xsl:text>
            </xsl:if>
            <xsl:value-of select="concat('djConfig.locale = ', $doubleQuote, $dojoLocale, $doubleQuote, ';')"/>
            var cocoon;
        ...
        <!--<script type="text/javascript" src="{$blockPrefix}/resource/js/dojotoolkit/dojo/dojo.js"/>-->
        <!-- load dojo -->
        <!--<script type="text/javascript">dojo.require("dojo.widget.*");</script>
         require dojo.widget for auto-loading
        <xsl:if test="$dojo-debug = 'true'">
            &lt;!&ndash; require console etc. for dojo debug, if requested &ndash;&gt;
            <script type="text/javascript">dojo.require("dojo.debug.console"); dojo.require("dojo.widget.Tree");
            </script>
        </xsl:if>-->
        <!--<script type="text/javascript">-->
        <!--<xsl:value-of select="concat('dojo.registerModulePath(', $doubleQuote, 'cocoon.forms', $doubleQuote, ', ', $doubleQuote, $forms-resources, '/js', $doubleQuote, ');')"/>-->
        <!--<xsl:value-of select="concat('dojo.registerModulePath(', $doubleQuote, 'cocoon.ajax', $doubleQuote, ', ', $doubleQuote, $forms-resources, '/js', $doubleQuote, ');')"/>-->

        <!--</script>-->
        <!--<script src="{$forms-resources}/js/forms.js" type="text/javascript"/>
        <script src="{$forms-resources}/js/forms-lib.js" type="text/javascript"/>
        <script src="{$forms-resources}/js/SimpleForm.js" type="text/javascript"/>
        <script src="{$forms-resources}/js/AjaxForm.js" type="text/javascript"/>
        <script src="{$ajax-resources}/js/BUHandler.js" type="text/javascript"/>-->
        <!-- load legacy scripts -->
        <!--<script type="text/javascript">                                            &lt;!&ndash; load forms library&ndash;&gt;
            <xsl:value-of
                    select="concat('dojo.registerModulePath(', $doubleQuote, 'cocoon.forms', $doubleQuote, ', ', $doubleQuote, $forms-resources, '/js', $doubleQuote, ');')"/>&ndash;&gt;
             tell dojo how to find our forms module. NB: (since 2.1.11, replaces cocoon.js)
            dojo.require("cocoon.forms.common");                                           &lt;!&ndash; tell dojo we require the commons library &ndash;&gt;
            dojo.require("cocoon.forms.manifest");                                         &lt;!&ndash; tell dojo we require cocoon.forms.manifest registering namespace handler &ndash;&gt;
            dojo.addOnLoad(cocoon.forms.callOnLoadHandlers);                               &lt;!&ndash; ask dojo to run our onLoad handlers &ndash;&gt;
        </script>-->

        <xsl:copy-of select="fi:init/node()"/>
        <!-- copy optional initialisation from form template -->
    ...
    <xsl:template match="fi:field[fi:selection-list]" priority="1">
        ...
        <!--<xsl:variable name="id" select="fi:id"/>-->
        ...
        <select title="{fi:hint}" id="{@id}" name="{@id}">
            ...
            <xsl:for-each select="fi:selection-list/fi:item">
                <option value="{@value}" id="{fi:label/node()}">
        ...
        <!-- dropdown or listbox -->
        <!--<span id="{@id}">-->
        <!--<select title="{fi:hint}" id="{@id}" name="{@id}" type="round_sb">-->

        <xsl:apply-templates select="." mode="common"/>
        <!--</span>-->
    </xsl:template>
    <xsl:template match="fi:field[fi:selection-list][fi:styling/@class='chooseOne']" priority="2">
        <xsl:variable name="value" select="fi:value"/>
        <!--<xsl:variable name="id" select="fi:id"/>-->
        <xsl:variable name="state" select="@state"/>
        <select title="{fi:hint}" id="{@id}" name="{@id}">
            ...
            <xsl:apply-templates select="." mode="styling"/>
            <option value="0" id="{fi:label/node()}">--</option>
            <xsl:for-each select="fi:selection-list/fi:item">
                <option value="{@value}" id="{fi:label/node()}">
                    <xsl:if test="@value = $value">
                        <xsl:attribute name="selected">selected</xsl:attribute>
    ...
    +-->
    <xsl:template match="fi:field[fi:selection-list][fi:styling/@list-type='radio']" priority="3">
    ...
    <xsl:template match="fi:field[fi:styling/@type='textarea']">
        <!--<span id="{@id}">-->
        <textarea id="{@id}" rows="15" cols="80" name="{@id}" title="{fi:hint}">
            <xsl:apply-templates select="." mode="styling"/>
    ...
    <!--+
    | fi:field with @type 'number'
    +-->
    <xsl:template match="fi:field[fi:styling/@type='number']" priority="2">
        <div class="common-div">
            <a role="button" class="ui-button ui-icon ui-icon-minus"
               onclick="javascript:process(-1, document.getElementById('{@id}').value)"/>
            <input type="text" size="5" render="yes" name="{@id}" id="{@id}" value="{fi:value}"/>
            <a role="button" class="ui-button ui-icon ui-icon-plus"
               onclick="javascript:process(1,  document.getElementById('{@id}').value)"/>
            <script language="javascript">
                function process(v,RowIndex){
                var exp = parseInt(document.getElementById('addedObjects.'+RowIndex+'.objectLevels').value)+v;
                <!--var exp = parseInt(document.getElementById('addedObjects-'+RowIndex+'-objectLevels').value)+v;-->
                if (exp >= 0){
    ...
    <xsl:template match="fi:field[fi:styling/@type='hidden']" priority="2">
        <input type="hidden" render="no" name="{@id}" id="{@id}" value="{fi:value}">
            <!--<xsl:apply-templates select="." mode="styling"/>-->
        </input>
    </xsl:template>
    ...
    +-->
    <xsl:template match="fi:field[@state='output']" priority="4">
        <span id="{@id}">
    ...
    <xsl:template match="fi:field[@state='output' and fi:styling/@type = 'link']" priority="5">
        <a id="{@id}" title="{fi:hint}" href="{fi:styling/@src}">
            <xsl:apply-templates select="." mode="css"/>
    ...
    <xsl:template match="fi:field[@state='output' and fi:selection-list]" priority="3">
        ...
        <span id="{@id}">
            <xsl:apply-templates select="." mode="css"/>
            <xsl:copy-of select="fi:selection-list/fi:item[@value=$value]/fi:label/node()"/>
        </span>
    </xsl:template>
    ...
    <xsl:template match="fi:validation-message">
        <div dojoType="cocoon.forms:InfoPopup" style="display:none" class="forms-validation-message-popup"
             id="forms-validation-message-{../@id}">
    ...
    <xsl:template match="fi:booleanfield">
        <!--<span id="{@id}">-->
        <input id="{@id}" type="checkbox" value="{@true-value}" name="{@id}" title="{fi:hint}">
            <xsl:apply-templates select="." mode="styling"/>
            <xsl:choose>
    ...
    <xsl:template match="fi:action">
        <!--<script type="text/javascript">
            $(function() {
            $( "#<xsl:value-of select="@id"/>-submit" ).button();
            });
        </script>-->
        <!--kremnef mod-->
        <input id="{@id}" type="submit" name="{@id}" title="{fi:hint}" value="{fi:label}">
    ...
    | fi:output is rendered as text
    +-->
    <xsl:template match="fi:output[fi:styling/@type = 'link']">
        <a id="{@id}" title="{fi:hint}" href="{fi:styling/@src}">
            <xsl:apply-templates select="." mode="css"/>
            <xsl:copy-of select="fi:value/node()"/>
        </a>
    ...
    <xsl:template match="fi:upload">
        ...
        <xsl:choose>
            <xsl:when test="fi:value">
                ...
                <xsl:text>]</xsl:text>
                <input type="button" id="{@id}" name="{@id}" value="..."
    ...
    <xsl:template match="fi:form-template|fi:form-generated">
        ...
        <div>
            <xsl:attribute name="id">
                <xsl:choose>
                    <xsl:when test="$id = 'dynamicForm'">
                        <xsl:value-of select="'modalDiv'"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="concat($id, 'Div')"/>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:attribute name="class">
                <!--<xsl:choose>
                <xsl:when test="$id = 'dynamicForm'">
                    <xsl:value-of select="'dialogContent'"/>
                </xsl:when>
                <xsl:otherwise>-->
        ...
        <xsl:if test="$messages">
            <xsl:variable name="header">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <p class="forms validation-errors-header">The following errors have been detected (marked with
                            !):
                        </p>
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:template match="fi:googlemap">
        ...
        <script type="text/javascript">
            <xsl:value-of
                    select="concat('var map_', $jsid, ' = new GMap2(document.getElementById(', $doubleQuote, @id, $doubleQuote, '),[G_HYBRID_MAP]);')"/>
            ...
            <xsl:value-of
                    select="concat('map_', $jsid, '.setCenter(new GLatLng(', fi:value/@lat, ', ', fi:value/@lng, '), ', fi:value/@zoom, ');')"/>

            <xsl:value-of
                    select="concat('GEvent.addListener(map_', $jsid, ', ', $doubleQuote, 'dragend', $doubleQuote, ', function()')"/>
            {
            <xsl:value-of
                    select="concat('document.getElementById(', $doubleQuote, @id, '_lng', $doubleQuote, ').setAttribute(', $doubleQuote, 'value', $doubleQuote, ',map_', $jsid, '.getCenter().x);')"/>
            <xsl:value-of
                    select="concat('document.getElementById(', $doubleQuote, @id, '_lat', $doubleQuote, ').setAttribute(', $doubleQuote, 'value', $doubleQuote, ',map_', $jsid, '.getCenter().y);')"/>
            });
            <xsl:value-of
                    select="concat('GEvent.addListener(map_', $jsid, ', ', $doubleQuote, 'zoomend', $doubleQuote, ', function(oldLevel,newLevel)')"/>
            {
            <xsl:value-of
                    select="concat('document.getElementById(', $doubleQuote, @id, '_zoom', $doubleQuote, ').value=newLevel;')"/>
            });
        ...
        <input name="{@id}_usermarker-lng" id="{@id}_usermarker-lng" value="{fi:value/fi:usermarker/@lng}"
               type="hidden"/>
        <input name="{@id}_usermarker-lat" id="{@id}_usermarker-lat" value="{fi:value/fi:usermarker/@lat}"
               type="hidden"/>
    </xsl:template>
    ...
    <xsl:template match="fi:value/fi:markers/fi:marker">
        ...
        <xsl:value-of
                select="concat('document.getElementById(', $doubleQuote, ../../../@id, '_current', $doubleQuote, ').value=', position())"/>
        });
    ...
    <xsl:template match="fi:value/fi:usermarker" mode="script">
        ...
        <xsl:value-of
                select="concat('var usermarker_', $jsid, ' = new GMarker(new GLatLng(', @lat, ', ', @lng, '));')"/>
        <xsl:value-of select="concat('map_', $jsid, '.addOverlay(usermarker_', $jsid, ');')"/>
        <xsl:value-of
                select="concat('GEvent.addListener(map_', $jsid, ', ', $doubleQuote, 'click', $doubleQuote, ', function(overlay,point)')"/>
        {
        <xsl:value-of select="concat('usermarker_', $jsid, '.setPoint(point);')"/>
        <xsl:value-of
                select="concat('document.getElementById(', $doubleQuote, ../../@id, '_usermarker-lng', $doubleQuote, ').value=point.x;')"/>
        <xsl:value-of
                select="concat('document.getElementById(', $doubleQuote, ../../@id, '_usermarker-lat', $doubleQuote, ').value=point.y;')"/>
        });
    ...
    <xsl:template match="fi:value/fi:usermarker" mode="geo">
        ...
        <input name="{../../@id}_geo_go" id="{../../@id}_geo_go" value="Go!"
               onclick="usermarker_{$jsid}.showAddress(this.form['{../../@id}_geo'].value)" type="button"/>
    </xsl:template>
    ...
    <xsl:template match="fi:*" mode="styling">
        ...
        <xsl:if test="@listening = 'true' and not(fi:styling/@submit-on-change = 'false') and
            ...
            <xsl:choose>
                ...
                <xsl:when
                        test="local-name() = 'booleanfield' or fi:styling/@list-type = 'radio' or fi:styling/@list-type = 'checkbox'">
