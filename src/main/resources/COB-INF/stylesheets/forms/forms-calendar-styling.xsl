    ...
    <xsl:template match="fi:field[fi:styling/@type='date'] |
        ...
        <!--<span id="{@id}">-->
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="$format = 'dd/MM/yyyy HH:mm'">
                            <script type="text/javascript">
                                $(function() {
                                    $("#<xsl:value-of select="@id"/>-input").datetimepicker({
                                    showButtonPanel: true, dateFormat:'dd/mm/yy'
                                })
                                    //showButtonPanel: true}, $.datepicker.regional[ "en-GB" ])
                        ...
                        <xsl:when test="$format = 'dd/MM/yyyy'">
                            <script type="text/javascript">
                                ...
                                $( "#<xsl:value-of select="@id"/>-input").datepicker({
                                numberOfMonths: 3,  dateFormat:'dd/mm/yy', showButtonPanel: true
                                });
                                });
                            </script>
                    ...
                    <input  id="{@id}-input" name="{@id}" value="{fi:value}" title="{normalize-space(fi:hint)}"
                           type="text"  datepattern="{$format}" variant="{$variant}">
