...
Form.prototype.sendFormAndWait = function(uri, viewdata, ttl) {
    ...
    var formName = this.form.getAttribute('formName');
    if (formName != null) {
        cocoon.session.setAttribute(formName + ".id", bookmark.id);
        cocoon.request.setAttribute(formName, this.form);
    ...
    if (comingBack) {
        ...
        var submitIdParameter = cocoon.request.getParameter(org.apache.cocoon.forms.formmodel.Form.SUBMIT_ID_PARAMETER);
        var widgets = {};
        var widgetStates = {};
        for (var iterator = this.form.getChildren(); iterator.hasNext();) {
            var formWidget = iterator.next();
            var fullName = formWidget.getFullName();
            if (formWidget instanceof org.apache.cocoon.forms.formmodel.Action && !fullName.equals(submitIdParameter)) {
                widgets[fullName] = formWidget; 
                widgetStates[fullName] = formWidget.getState();
                formWidget.setState(org.apache.cocoon.forms.formmodel.WidgetState.DISABLED);
            }
        }
        finished = this.form.process(formContext);
        for (fullName in widgets) {
            widgets[fullName].setState(widgetStates[fullName]);
        }
        if (finished) {
            this.isValid = this.form.isValid();
            var widget = this.form.getSubmitWidget();
			...
			//html as demonstrated by IframeTransport-bu-styling.xsl
	                    ...
	                    text = "<?xml version='1.0'?><bu:document ver=\"kkkk\" xmlns:bu='" +
